...
model{
  ...
  for(t in 1:(nyrs-1)){
    ...
    c.rand[t] ~ dnorm(c[t], tau.c)
    N.est[t+1] <- lambda[t]*(N.est[t] - c.rand[t])
  }
...
wildeparms <- c("beta0", "beta1", "sig.c", "sig.r", "lambda", "N.est")
