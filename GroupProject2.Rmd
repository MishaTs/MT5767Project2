...
A state-space model (SSM) consists of two components: an unobserved "true" population process, $N_t$, and the observation process, $y_t$, that often entails observation process errors and uncertainties. Here, true wildebeest population $N_t$ depended on the population growth rate, $\lambda_t$, after losses from annual poaching, $c_t$. To account for random variation common in natural systems, large $N_{t+1}$s take a normal distribution consistent with the thousands of wildebeest observed. $$N_{t+1} \sim N(\lambda_t*(N_t - c_t), \sigma^2_t)$$
...
Known data include rainfall $R_t$ and poaching $c_t$ for all years as well as population estimates $y_t$ and associated error $se_t$ for `r length(validObs)` years. These underly Bayesian estimation for unknown parameters $\beta_0$, $beta_1$, and $\sigma_t$; these values further provide estimates for population growth rates $\lambda_t$ and true wildebeest population dynamics $N_{t}$. 
...
```{r}
##this code is a 3x duplicate. do we need it?
citation("jagsUI")
# necessary dependencies
