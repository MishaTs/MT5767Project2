...
```{r regOutput, echo = FALSE}
regOutput <- data.frame(coef(summary(glsExpFTested)))
regCleaned <- regOutput %>% mutate(Value = round(Value^2 * ifelse(Value<0,-1,1),3),
                                   Std.Error = round(`Std.Error`,3),
                                   t.value = round(`t.value`,1),
                                   p.value = round(`p.value`,2))
rownames(regCleaned) <- c("Intercept",
                           "Flooded tide",
                           "Slack tide",
                           "Hour of day",
                           "Construction stage",
                           "x position")
knitr::kable(regCleaned, align="c",
             format = "latex",
             col.names = c("Regressor" ,
                           "Effect Magnitude",
                           "Standard Error",
                           "t Value",
                           "p Value"),
             caption = "GLS Regression Model Output") %>% 
  kable_classic(full_width = F) %>% 
  footnote(general_title = "Note:", 
           footnote_as_chunk = TRUE,
           threeparttable = TRUE,
           general = "Effect Magnitude is the impact of a unit change in a regressor on animal density (an increase if positive and a decrease if negative). Standard error signifies the uncertainty surrounding the effect magnitudes, which are measured using t and p values. If p < 0.05, then re-estimating the effect will result in a nonzero value at least 95% of the time. Here, the x position effect is technically non-zero but very small. Construction stage shows a one-off effect of conducting construction, and effects from tidestates are relative to ebb tides.")
```